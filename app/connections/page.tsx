...
export default function Home() {
  ...
  const groupAnswers = {
    yellow: (
      <div className=" animate-popUp mt-2 flex-col font-sans h-24 text-xl w-full bg-nyt-yellow rounded-xl flex justify-center items-center">
        <div className="flex-col font-bold text-center">
          CONNECT
          <div className="font-normal">COUPLE, JOIN, LINK, TIE</div>
        </div>
      </div>
    ),
    green: (
      <div className=" animate-popUp mt-2 flex-col font-sans h-24 text-xl w-full bg-nyt-green rounded-xl flex justify-center items-center">
        <div className="flex-col font-bold text-center">
          NUISANCE
          <div className="font-normal">BOTHER, HANDFUL, PAIN, PEST</div>
        </div>
      </div>
    ),
    blue: (
      <div className=" animate-popUp mt-2 flex-col font-sans h-24 text-xl w-full bg-nyt-blue rounded-xl flex justify-center items-center">
        <div className="flex-col font-bold text-center">
          QUANTITY WORDS
          <div className="font-normal">FEW, MANY, SEVERAL, SOME</div>
        </div>
      </div>
    ),
    purple: (
      <div className=" animate-popUp mt-2 flex-col font-sans h-24 text-xl w-full bg-nyt-purple rounded-xl flex justify-center items-center">
        <div className="flex-col font-bold text-center">
          TAROT CARDS, WITH "THE"
          <div className="font-normal">FOOL, LOVERS, MAGICIAN, TOWER</div>
        </div>
      </div>
    ),
  };
  ...
  return (
    <main className="w-full h-full">
      ...
      <button
        onClick={onOpen}
        className="hover:shadow-2xl flex ml-auto hover:bg-slate-200 ease-in-out duration-150 p-1 m-2 text-4xl"
      >
      ...
      <div className="text-xl flex   mx-auto justify-center text-center">
      ...
      <div className="  mt-7 w-connectionW h-allConnections mx-auto  text-center">
        ...
        <form>
          <div className="ease-in-out duration-75 gap-x-2 grid grid-cols-4">
            {quantityWords.map((word, index) => (
              <button
                ...
                value={word}
                className={`${
                  selectedWords.includes(word) ? "bg-nyt-grayer" : "bg-nyt-gray"
                } ${
                  selectedWords.includes(word) ? "text-white" : "text-black"
                } select-none ease-out duration-75 rounded-xl mt-2 h-10 lg:h-24 items-center justify-center flex text-md lg:text-xl bg-nyt-gray font-bold`}
              >
                <label
          ...
          <div className="m-7 text-xl items-center justify-center flex">
            ...
            <span
              className={` ${
                lives >= 1 ? "visible" : "animate-shrink "
              } w-5 h-5 mx-2 inline-block rounded-full bg-nyt-grayer`}
            ></span>
            <span
              className={` ${
                lives >= 2 ? "visible" : "animate-shrink "
              } w-5 h-5 mx-2 inline-block rounded-full bg-nyt-grayer`}
            ></span>
            <span
              className={` ${
                lives >= 3 ? "visible" : "animate-shrink "
              } w-5 h-5 mx-2 inline-block rounded-full bg-nyt-grayer`}
            ></span>
            <span
              className={` ${
                lives >= 4 ? "visible" : "animate-shrink "
              } w-5 h-5 mx-2 inline-block rounded-full bg-nyt-grayer`}
            ></span>
          </div>
          {correctAnswers.length != 4 ? (
            <div className="justify-center font-semibold text-lg flex gap-x-8">
              <button
                onClick={shuffle}
