...
export default function Home() {
  ...
  const checkAnswer = async (e: { preventDefault: () => void }) => {
    ...
    if (y) {
      ...
      updateUser(user?.email ?? '', { visy: true, quantityWords: ans, correctAnswersDisplay: newDisplay });
    } else if (g) {
      setCorrectAnimation(true);
      setTimeout(() => {
        setCorrectAnimation(false);
      ...
      const newDisplay = correctAnswersDisplay
      updateUser(user?.email ?? '', { visg: true, quantityWords: ans, correctAnswersDisplay: newDisplay });
    } else if (b) {
      setCorrectAnimation(true);
      setTimeout(() => {
        setCorrectAnimation(false);
      ...
      const newDisplay = correctAnswersDisplay
      updateUser(user?.email ?? '', { visb: true, quantityWords: ans, correctAnswersDisplay: newDisplay });
    } else if (p) {
      setCorrectAnimation(true);
      setTimeout(() => {
        setCorrectAnimation(false);
      ...
      const newDisplay = correctAnswersDisplay
      updateUser(user?.email ?? '', { visp: true, quantityWords: ans, correctAnswersDisplay: newDisplay });
    } else {
      ...
      if (newLives == 0) {
        ...
        if (!visy) {
          ...
          correctAnswersDisplay.push("yellow");
          const newDisplay = correctAnswersDisplay
          updateUser(user?.email ?? '', { visy: true, quantityWords: ans1, correctAnswersDisplay: newDisplay });
          await delay(1200);
        } if (!visg) {
          await delay(800);
          setVisg(true);
          ...
          correctAnswersDisplay.push("green");
          const newDisplay = correctAnswersDisplay
          updateUser(user?.email ?? '', { visg: true, quantityWords: ans2, correctAnswersDisplay: newDisplay });
          await delay(1200);
        } if (!visb) {
          await delay(800);
          setVisb(true);
          ...
          correctAnswersDisplay.push("blue");
          const newDisplay = correctAnswersDisplay
          updateUser(user?.email ?? '', { visb: true, quantityWords: ans3, correctAnswersDisplay: newDisplay });
          await delay(1200);
        } if (!visp) {
          await delay(800);
          setVisp(true);
          ...
          correctAnswersDisplay.push("purple");
          const newDisplay = correctAnswersDisplay
          updateUser(user?.email ?? '', { visp: true, quantityWords: ans4, correctAnswersDisplay: newDisplay });
          await delay(1200);
        }
      }
    }
